...
int LCCL_ClientManager_HandleRequest(LCCL_CLIENTMANAGER *clm,
  ...
  /* Insert more handlers here */
    ...
    DBG_DEBUG(0, "Command \"%s\" not handled by client manager", name);
    rv=1; /* not handled */
  }
...
void LCCL_ClientManager_NewCard(LCCL_CLIENTMANAGER *clm, LCCO_CARD *card){
  if (LCCO_Card_GetStatus(card)==LC_CardStatusInserted) {
    ...
    LCCL_ClientManager_SendCardNotification(clm, 0,
...
void LCCL_ClientManager_RemoveClientRequests(LCCL_CLIENTMANAGER *clm,
  ...
  if (rql) {
	...
	if (cl==NULL || cl==LCCL_Request_GetClient(rq)) {
	  ...
	  subrql=GWEN_IpcRequest_GetSubRequests(rq);
	  if (subrql)
	    LCCL_ClientManager_RemoveClientRequests(clm, NULL, subrql);

	  /* remove request from IPC manager */
	  rid=GWEN_IpcRequest_GetId(rq);
	  assert(rid);
	  DBG_INFO(0, "Removing request %x\n", rid);
	  if (GWEN_IpcManager_RemoveRequest(clm->ipcManager, rid, 0)) {
	    DBG_ERROR(0, "Could not remove request");
	  }

	  /* free request */
	  GWEN_IpcRequest_List_Del(rq);
	  GWEN_IpcRequest_free(rq);
	}
	rq=rqNext;
      }
    }
...
void LCCL_ClientManager_ClientDown(LCCL_CLIENTMANAGER *clm,
  ...
  /* remove all requests of this client (including subrequests) */
  rm=LCS_Server_GetRequestManager(clm->server);
  assert(rm);
  rql=GWEN_IpcRequestManager_GetRequests(rm);
  if (rql)
    LCCL_ClientManager_RemoveClientRequests(clm, cl, rql);
...
#include "clr_clientready.c"
#include "clr_notification.c"
#include "clr_startwait.c"
#include "clr_stopwait.c"
#include "clr_takecard.c"
#include "clr_releasecard.c"
#include "clr_execapdu.c"
#include "clr_getdrivervar.c"
#include "clr_lockreader.c"
#include "clr_unlockreader.c"
#include "clr_readercmd.c"
