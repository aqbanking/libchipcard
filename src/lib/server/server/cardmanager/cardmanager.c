...
void LCCM_CardManager_CardRemoved(LCCM_CARDMANAGER *cm, LCCO_CARD *card) {
  DBG_DEBUG(0, "===================== Card %08x/%d/%08x removed",
            LCCO_Card_GetReaderId(card),
            LCCO_Card_GetSlotNum(card),
            LCCO_Card_GetCardNum(card));
  LCCO_Card_SetStatus(card, LC_CardStatusRemoved);
}
...
int LCCM_CardManager_Work(LCCM_CARDMANAGER *cm){
  ...
  while(card) {
    ...
    if (currSt!=lastSt) {
      ...
      DBG_INFO(0, "Status of card \"%08x\" changed",
      ...
      if (t) {
        ...
        if (difftime(t1, t)>cm->unusedTimeout) {
          ...
          if (difftime(t1, t)>cm->minimumKeepTime) {
            DBG_INFO(0, "Keep time exceeded");

            DBG_DEBUG(0, "Allowing reader to shut down");
    ...
    if (currSt!=LC_CardStatusInserted) {
      if (currSt!=lastSt) {
        DBG_INFO(0, "Card %08x/%d/%08x removed (%d)",
                 LCCO_Card_GetReaderId(card),
                 LCCO_Card_GetSlotNum(card),
      ...
      if (LCCM_Card_HasLockRequests(card)==0) {
        ...
        DBG_ERROR(0, "Free card no longer inserted, removing from list");
