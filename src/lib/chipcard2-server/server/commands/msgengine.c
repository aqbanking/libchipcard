...
int LC_MsgEngine_TypeRead(GWEN_MSGENGINE *e,
  ...
  if (strcasecmp(type, "byte")==0) {
    ...
    if (1!=sscanf(GWEN_XMLNode_GetProperty(node, "size", "-1"),
                  "%i", &size)) {
      DBG_ERROR(0, "Bad number of bytes");
      return -1;
    }
    if (size==-1) {
      size=GWEN_Buffer_GetBytesLeft(msgbuf);
      if (size==0) {
        DBG_INFO(0, "No bytes found");
        return 0;
      }
    }
    else {
      if (size>GWEN_Buffer_GetBytesLeft(msgbuf)) {
        DBG_ERROR(0, "Too few bytes in message (%d>%d)",
                  size, GWEN_Buffer_GetBytesLeft(msgbuf));
        return -1;
      }
    }
    ...
    if (GWEN_Text_ToBcdBuffer(GWEN_Buffer_GetPosPointer(msgbuf),
      ...
      DBG_ERROR(0, "Error parsing BCD string");
      return -1;
    }
    ...
    DBG_DEBUG(0, "Tag data length is %d (total %d)", j, pos);
    if (GWEN_Buffer_AppendBytes(vbuf,
                                GWEN_Buffer_GetPosPointer(msgbuf),
                                pos)) {
      DBG_INFO(0, "here");
    ...
    DBG_DEBUG(0, "Type \"%s\" not supported by LC_MsgEngine", type);
    return 1;
  }
}
...
int LC_MsgEngine_TypeWrite(GWEN_MSGENGINE *e,
  ...
  if (strcasecmp(type, "byte")==0) {
    ...
    DBG_DEBUG(0, "Supporting type \"byte\"");
    isBCD=atoi(GWEN_XMLNode_GetProperty(node, "bcd", "0"));
    if (1!=sscanf(GWEN_Buffer_GetPosPointer(data), "%i", &value)) {
      DBG_ERROR(0, "Bad number");
      return -1;
    }

    if (value>255 || value<0) {
      DBG_ERROR(0, "Number out of range (%d)", value);
      return -1;
    }
    ...
    DBG_DEBUG(0, "Supporting type \"word\"");
    isBCD=atoi(GWEN_XMLNode_GetProperty(node, "bcd", "0"));
    if (1!=sscanf(GWEN_Buffer_GetPosPointer(data), "%i", &value)) {
      DBG_ERROR(0, "Bad number");
      return -1;
    ...
    DBG_DEBUG(0, "Supporting type \"dword\"");
    isBCD=atoi(GWEN_XMLNode_GetProperty(node, "bcd", "0"));
    if (1!=sscanf(GWEN_Buffer_GetPosPointer(data), "%i", &value)) {
      DBG_ERROR(0, "Bad number");
      return -1;
    ...
    for (i=0; i<pinlen; i++) {
      ...
      j=p[i]-'0';
      if (j>9) {
        DBG_ERROR(0, "Only digits allowed in FPIN2");
        return -1;
      }
    ...
    if (size==-1) {
      ...
      if (size>GWEN_Buffer_GetUsedBytes(data)) {
        ...
        if (lfs) {
          DBG_NOTICE(0, "Filler is \"%s\"", lfs);
          if (1!=sscanf(lfs, "%i", &lfiller)) {
            DBG_ERROR(0, "Bad value for property lfiller");
            return -1;
          }
          DBG_NOTICE(0, "Filling with %02x", (unsigned char)lfiller);
    ...
    DBG_ERROR(0, "Written %d bytes", GWEN_Buffer_GetUsedBytes(gbuf));
    return 0;
  }
...
int LC_MsgEngine_BinTypeRead(GWEN_MSGENGINE *e,
  ...
  if (strcasecmp(typ, "tlv")==0) {
    ...
    DBG_VERBOUS(0, "Entering BinTypeRead with this:");
    ...
    while (tlvNode) {
      ...
      if (1!=sscanf(GWEN_XMLNode_GetProperty(tlvNode, "id", "-1"),
                    "%i", &ltagType)) {
        DBG_WARN(0, "Bad tag id in XML file");
      }
      else {
        if (ltagType==tagType) {
          DBG_DEBUG(0, "Tag %02x found in XML file", ltagType);
    ...
    DBG_INFO(0, "Tag \"%02x\" not found", tagType);
    name=GWEN_XMLNode_GetProperty(node, "name", 0);
    ngr=gr;
    if (name) {
      if (*name) {
