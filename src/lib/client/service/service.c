...
LC_CLIENT *LC_Service_new(int argc, char **argv) {
  ...
  cres=LC_Client_Init(cl);
  if (cres!=LC_Client_ResultOk) {
    DBG_INFO(0, "Could not init libchipcard (%d)", cres);
...
int LC_Service_HandleServiceOpen(LC_CLIENT *cl,
  ...
  if (1!=sscanf(GWEN_DB_GetCharValue(dbReq, "body/clientId", 0, "0"),
    ...
    LC_ClientLcc_DeleteInRequest(cl, rid);
    return -1;
  }
  ...
  if (LC_ClientLcc_SendResponse(cl, rid, dbRsp)) {
...
int LC_Service_Work(LC_CLIENT *cl) {
  ...
  while(!sv->stopService) {
    ...
    res=LC_ClientLcc_Work_Wait(cl, 10);
...
int LC_Service_Connect(LC_CLIENT *cl, const char *code, const char *text){
  ...
  rid=LC_ClientLcc_SendRequest(cl,
                               0, /* no card */
                               0, /* all servers */
                               dbReq);
  if (rid==0) {
