...
int doActivateKey(LC_CLIENT *cl,
                  LC_CARD *card,
                  GWEN_DB_NODE *dbArgs) {
  ...
  if (GWEN_KeySpec_GetStatus(ks)!=0x08) {
    fprintf(stderr,
            "Key already exists, please remove it first by using the\n"
            "command \"deactivatekey\"\n");
    GWEN_KeySpec_free(ks);
    return RETURNVALUE_WORK;
  }

  DBG_NOTICE(0, "Activating key %02x", dstkid);
  ...
  GWEN_KeySpec_SetNumber(ks, GWEN_DB_GetIntValue(dbArgs, "keyNumber", 0, 1));
  GWEN_KeySpec_SetVersion(ks,
                          GWEN_DB_GetIntValue(dbArgs, "keyVersion", 0, 1));

  if (v>0)
    fprintf(stderr, "Activating key.\n");
  res=LC_Starcos_ActivateKeyPair(card, srckid, dstkid, ks);
  GWEN_KeySpec_free(ks);
  if (res!=LC_Client_ResultOk) {
    showError(card, res, "ActivateKeyPair");
    return RETURNVALUE_WORK;
  }
  if (v>0)
    fprintf(stderr, "Key activated.\n");
...
int activateKey(LC_CLIENT *cl, GWEN_DB_NODE *dbArgs) {
  ...
  rv=doActivateKey(cl, card, dbArgs);
