 ...
 */
static int _driverFindFile(GWEN_STRINGLIST *slDirs, GWEN_STRINGLIST *slNames, GWEN_BUFFER *nbuf);
static GWEN_DB_NODE *_driverDbFromXml(GWEN_XMLNODE *node, int dontSearchDrivers);
static GWEN_DB_NODE *_readerDbFromXml(GWEN_XMLNODE *node);
static int _readDriverFile(const char *fname, GWEN_DB_NODE *dbDrivers, int availOnly, int dontSearchDrivers);
  ...
  if (!GWEN_Directory_Open(d, GWEN_Buffer_GetStart(buf))) {
    ...
    while (!GWEN_Directory_Read(d, buffer, sizeof(buffer))) {
      ...
      if (stat(GWEN_Buffer_GetStart(buf), &st)) {
        DBG_ERROR(LC_LOGDOMAIN, "stat(%s): %s", GWEN_Buffer_GetStart(buf), strerror(errno));
      }
      else {
        if (!S_ISDIR(st.st_mode)) {
          if (flen>3) {
            if (strncasecmp(buffer+flen-4, ".xml", 4)==0) {
              DBG_DEBUG(0, "Reading driver file \"%s\"", GWEN_Buffer_GetStart(buf));
              if (!_readDriverFile(GWEN_Buffer_GetStart(buf), dbDrivers, availOnly, dontSearchDrivers))
                drivers++;
            }
          }
    ...
    DBG_INFO(LC_LOGDOMAIN, "Could not open folder [%s]", GWEN_Buffer_GetStart(buf));
  }
...
int _driverFindFile(GWEN_STRINGLIST *slDirs, GWEN_STRINGLIST *slNames, GWEN_BUFFER *nbuf)
{
    ...
    while (eNames) {
      ...
      if (!GWEN_Directory_Open(dDir, GWEN_StringListEntry_Data(eDirs))) {
        ...
        while (!GWEN_Directory_Read(dDir, nameBuf, sizeof(nameBuf))) {
          if (strcmp(nameBuf, ".")!=0 &&
              strcmp(nameBuf, "..")!=0) {
            if (-1!=GWEN_Text_ComparePattern(nameBuf, GWEN_StringListEntry_Data(eNames), 0)) {
              struct stat st;
              ...
              if (stat(GWEN_Buffer_GetStart(nbuf), &st)) {
                /* error */
                DBG_WARN(0, "stat(%s): %s", GWEN_Buffer_GetStart(nbuf), strerror(errno));
              }
              else {
                ...
                if (S_ISREG(st.st_mode)) {
                  ...
                  DBG_INFO(0, "Entry \"%s\" is not a regular file", GWEN_Buffer_GetStart(nbuf));
                }
              }
...
int _readDriverFile(const char *fname, GWEN_DB_NODE *dbDrivers, int availOnly, int dontSearchDrivers)
{
  GWEN_XMLNODE *nFile;
  int driverAdded=0;

  nFile=GWEN_XMLNode_new(GWEN_XMLNodeTypeTag, "drivers");
  if (GWEN_XML_ReadFile(nFile, fname, GWEN_XML_FLAGS_DEFAULT)) {
    ...
    if (!nDriver) {
      ...
      dbDriver=_driverDbFromXml(nDriver, dontSearchDrivers);
      if (!dbDriver) {
        DBG_INFO(0, "Could not create driver from file \"%s\"", fname);
      }
      else {
        if (GWEN_DB_GetCharValue(dbDriver, "libraryFile", 0, 0) || !availOnly || dontSearchDrivers) {
          GWEN_XMLNODE *nReader;

          nReader=GWEN_XMLNode_FindFirstTag(nDriver, "readers", "osname", OS_SHORTNAME);
          if (!nReader)
            nReader=GWEN_XMLNode_FindFirstTag(nDriver, "readers", "ostype", OS_TYPE);
          if (!nReader) {
            ...
            while (nReader) {
              if (GWEN_XMLNode_GetProperty(nReader, "osname", 0)==0 &&
                ...
                break;
              nReader=GWEN_XMLNode_FindNextTag(nReader, "readers", 0, 0);
            } /* while */
          }
          if (!nReader) {
            ...
            while (nReader) {
              ...
              dbReader=_readerDbFromXml(nReader);
              if (dbReader) {
...
GWEN_DB_NODE *_driverDbFromXml(GWEN_XMLNODE *node, int dontSearchDrivers)
{
  GWEN_DB_NODE *db;
  GWEN_XMLNODE *n;
  GWEN_XMLNODE *nLib;
  ...
  GWEN_DB_SetCharValue(db, GWEN_DB_FLAGS_DEFAULT, "driverName", dname);

  n=GWEN_XMLNode_FindFirstTag(node, "manufacturer", 0, 0);
  if (n) {
    p=GWEN_XMLNode_GetCharValue(n, "name", 0);
    if (p)
      GWEN_DB_SetCharValue(db, GWEN_DB_FLAGS_DEFAULT, "manufacturer", p);
    p=GWEN_XMLNode_GetCharValue(n, "url", 0);
    if (p)
      GWEN_DB_SetCharValue(db, GWEN_DB_FLAGS_DEFAULT, "url", p);
  }
  ...
  if (n) {
    ...
    while (nn) {
      ...
      if (nd)
        ...
        value="";
      GWEN_DB_SetCharValue(dbVars, GWEN_DB_FLAGS_DEFAULT, name, value);
  ...
  GWEN_DB_SetCharValue(db, GWEN_DB_FLAGS_DEFAULT, "driverType", p);
  GWEN_DB_SetIntValue(db, GWEN_DB_FLAGS_DEFAULT, "maxReaders", GWEN_XMLNode_GetIntProperty(node, "maxReaders", 1));

  p=GWEN_XMLNode_GetCharValue(node, "short", 0);
  if (p)
    GWEN_DB_SetCharValue(db, GWEN_DB_FLAGS_DEFAULT, "short", p);
    ...
    if (nLib) {
      ...
      while (n) {
        ...
        if (n) {
          ...
          if (p)
            GWEN_StringList_AppendString(slDirs, p, 0, 1);
        }
      ...
      while (n) {
        ...
        if (n) {
          p=GWEN_XMLNode_GetData(nData);
          if (p)
            GWEN_StringList_AppendString(slNames, p, 0, 1);
        }
        n=GWEN_XMLNode_FindNextTag(n, "name", 0, 0);
      } /* while */

      nbuf=GWEN_Buffer_new(0, 256, 0, 1);
      if (!_driverFindFile(slDirs, slNames, nbuf))
        GWEN_DB_SetCharValue(db, GWEN_DB_FLAGS_DEFAULT, "libraryFile", GWEN_Buffer_GetStart(nbuf));
      GWEN_Buffer_free(nbuf);
      GWEN_StringList_free(slNames);
      GWEN_StringList_free(slDirs);
    }
    else
      GWEN_DB_SetCharValue(db, GWEN_DB_FLAGS_DEFAULT, "libraryFile", "none");
  }
  ...
  if (n) {
    ...
    while (nn) {
      ...
      if (nd)
        ...
        value="";
      GWEN_DB_SetCharValue(db, GWEN_DB_FLAGS_DEFAULT, "flags", value);
...
GWEN_DB_NODE *_readerDbFromXml(GWEN_XMLNODE *node)
{
  GWEN_DB_NODE *db;
  GWEN_XMLNODE *n;
  const char *rtype;
  ...
  GWEN_DB_SetCharValue(db, GWEN_DB_FLAGS_DEFAULT, "readerType", rtype);

  p=GWEN_XMLNode_GetCharValue(node, "short", rtype);
  GWEN_DB_SetCharValue(db, GWEN_DB_FLAGS_DEFAULT, "shortName", p);

  p=GWEN_XMLNode_GetProperty(node, "busType", "serial");
  GWEN_DB_SetCharValue(db, GWEN_DB_FLAGS_DEFAULT, "busType", p);
  GWEN_DB_SetIntValue(db, GWEN_DB_FLAGS_DEFAULT, "slots", GWEN_XMLNode_GetIntProperty(node, "slots", 1));
  GWEN_DB_SetIntValue(db, GWEN_DB_FLAGS_DEFAULT, "ctn", GWEN_XMLNode_GetIntProperty(node, "ctn", 1));

  if (1==sscanf(GWEN_XMLNode_GetProperty(node, "busId", "-1"), "%i", &i))
    if (i!=-1)
      GWEN_DB_SetIntValue(db, GWEN_DB_FLAGS_DEFAULT, "busId", i);
  if (1==sscanf(GWEN_XMLNode_GetProperty(node, "vendor", "-1"), "%i", &i))
    if (i!=-1)
      GWEN_DB_SetIntValue(db, GWEN_DB_FLAGS_DEFAULT, "vendorId", i);
  if (1==sscanf(GWEN_XMLNode_GetProperty(node, "product", "-1"), "%i", &i))
    if (i!=-1)
      GWEN_DB_SetIntValue(db, GWEN_DB_FLAGS_DEFAULT, "productId", i);

  if (1==sscanf(GWEN_XMLNode_GetProperty(node, "usbclass", "-1"), "%i", &i))
    if (i!=-1)
      GWEN_DB_SetIntValue(db, GWEN_DB_FLAGS_DEFAULT, "usbclass", i);
  ...
  if (n) {
    ...
    while (n) {
      ...
      if (nData) {
        p=GWEN_XMLNode_GetData(nData);
        if (p)
          GWEN_DB_SetCharValue(db, GWEN_DB_FLAGS_DEFAULT, "flags", p);
      }
    ...
    while (n) {
      ...
      if (1!=sscanf(vp, "%i", &i)) {
        ...
        if (nData) {
          p=GWEN_XMLNode_GetData(nData);
          if (p)
            GWEN_DB_SetIntValue(dbPorts, GWEN_DB_FLAGS_DEFAULT, p, i);
        }
        else {
  ...
  if (n) {
    ...
    dbAutoPort=GWEN_DB_GetGroup(db, GWEN_DB_FLAGS_OVERWRITE_GROUPS, "autoport");
    s=GWEN_XMLNode_GetCharValue(n, "mode", 0);
    if (s)
      GWEN_DB_SetCharValue(dbAutoPort, GWEN_DB_FLAGS_DEFAULT, "mode", s);
    ...
    if (s) {
      ...
      while (psc && *psc) {
        ...
        if (p)
          *(p++)=0;
        GWEN_DB_SetCharValue(dbAutoPort, GWEN_DB_FLAGS_DEFAULT, "busorder", psc);
        psc=p;
      }
      free(scopy);
    }
    ...
    if (s)
      GWEN_DB_SetCharValue(dbAutoPort, GWEN_DB_FLAGS_DEFAULT, "sortkey", s);

    /* offset */
    i=GWEN_XMLNode_GetIntValue(n, "offset", 0);
    GWEN_DB_SetIntValue(dbAutoPort, GWEN_DB_FLAGS_DEFAULT, "offset", i);

  }

  p=GWEN_XMLNode_GetProperty(node, "addressType", "port");
  assert(p);
  GWEN_DB_SetCharValue(db, GWEN_DB_FLAGS_DEFAULT, "addressType", p);

  p=GWEN_XMLNode_GetProperty(node, "devicePathTmpl", 0);
  if (p)
    GWEN_DB_SetCharValue(db, GWEN_DB_FLAGS_DEFAULT, "devicePathTmpl", p);
